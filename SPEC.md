...
**追加授权（single-writer space）**：创世 delta 的签名者成为该空间的 recorded agent；非创世 delta 的恢复签名者**必须**等于当前 head 的 agent，否则任何观察到公开 head 的人都能用自己的合法签名接管别人的空间（进而劫持记忆市场里以 head agent 为准的所有权）。多写者/所有权转移留给 ERC-8264 权利 / ERC-8312 mandate 策略扩展。
...
- **追加授权 / 空间劫持**：head 是公开的；若注册表接受任意合法签名的非创世 delta，第三方即可延长并接管他人空间。必须强制 single-writer 规则（非创世签名者 = head agent）或等效显式授权策略。
