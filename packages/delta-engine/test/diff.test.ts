...
import { commitContent, diffStates, stableStringify, type MemoryEntry } from "../src/index.js";
...
describe("stableStringify / commitContent", () => {
  it("is key-order independent", () => {
    expect(stableStringify({ b: 1, a: 2 })).toBe(stableStringify({ a: 2, b: 1 }));
    expect(commitContent({ b: 1, a: 2 })).toBe(commitContent({ a: 2, b: 1 }));
  });
  it("tolerates floats (for embedding payloads)", () => {
    expect(commitContent({ v: [0.1, 0.2] })).toMatch(/^0x[0-9a-f]{64}$/);
  });
});
...
describe("diffStates", () => {
  ...
  it("update prior commitment equals the previous content commitment", () => {
    ...
    if (op.op === "update") expect(op.priorCommitment).toBe(commitContent({ text: "x" }));
  });
