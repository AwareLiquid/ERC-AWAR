...
import { fingerprintContent, type MemoryEntry } from "./record.js";
...
export function diffStates(prev: MemoryEntry[], next: MemoryEntry[]): DiffOp[] {
  ...
  for (const [id, entry] of nextMap) {
    const commitment = fingerprintContent(entry.content);
    const before = prevMap.get(id);
    if (!before) {
      ops.push({ op: "add", id, entry, commitment, priorCommitment: ZERO32 });
      continue;
    }
    const priorCommitment = fingerprintContent(before.content);
  ...
  for (const [id, entry] of prevMap) {
    if (!nextMap.has(id)) {
      ops.push({ op: "deprecate", id, priorCommitment: fingerprintContent(entry.content) });
    }
  }
